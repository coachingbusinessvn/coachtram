...
function initBookingForm() {
  ...
  if (!form || form.dataset.bookingBound) return;
  ...
  if (selectedService) {
    const radio = controls
      .find(input => input.value === selectedService);
  ...
  const updateState = () => {
    ...
    if (status) {
      status.textContent = isValid
        ? 'Sẵn sàng gửi questionnaire.'
        : 'Điền đủ các mục bắt buộc để bật nút Submit.';
    }
  };
  ...
  form.addEventListener('submit', (event) => {
    ...
    localStorage.setItem('edinaBookingQuestionnaire', JSON.stringify(data));

    const calendarUrl = form.dataset.calendarUrl?.trim();
    if (calendarUrl) {
    ...
    if (status) status.textContent = 'Đã lưu questionnaire trên trình duyệt này.';
